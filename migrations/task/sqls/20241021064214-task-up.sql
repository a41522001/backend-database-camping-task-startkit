...
('肌肉棒子', 'muscle@hexschooltest.i', 'USER'),
...
set role = 'COACH'
where 
	name = (select name from "USER" where email = 'lee2000@hexschooltest.io') or
	name = (select name from "USER" where email = 'muscle@hexschooltest.i') or
	name = (select name from "USER" where email = 'starplatinum@hexschooltest.io');
...
(
	(select id from "USER" WHERE email = (select email from "USER" where name = '肌肉棒子')), 2
),
(
	(select id from "USER" WHERE email = (select email from "USER" where name = 'Q太郎')), 2
);
...
select 
	SUM("CREDIT_PACKAGE".credit_amount) as total,
	"USER".id as user_id
...
select 
	...
	COUNT("CREDIT_PACKAGE".name) as 銷售數量
from "CREDIT_PURCHASE"
inner join "CREDIT_PACKAGE" on "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id
where "CREDIT_PACKAGE".created_at >= '2024-12-01'
...
-- 顯示須包含以下欄位： 預約會員人數
select
	COUNT(DISTINCT "USER".name) as 預約會員人數
from "COURSE_BOOKING"
inner join "USER" on "USER".id = "COURSE_BOOKING".user_id 
where "COURSE_BOOKING".status not IN ('課程已取消') and "COURSE_BOOKING".created_at >= '2024-12-01';
